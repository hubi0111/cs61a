...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    num_dice, avg_score, avg = 0, 0, make_averaged(roll_dice, trials_count)
    for i in range(1, 11):
        avg2 = avg(i, dice)
        if avg2 > avg_score:
            num_dice, avg_score = i, avg2
    return num_dice
    # END PROBLEM 9
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    point_diff = opponent_score - score
    if free_bacon(opponent_score) >= points_to_win or extra_turn(
        score + free_bacon(opponent_score), opponent_score
    ):
        return 0
    if point_diff < 0:
